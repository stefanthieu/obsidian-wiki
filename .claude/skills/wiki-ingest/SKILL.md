...
In raw mode, each file in `OBSIDIAN_VAULT_PATH/_raw/` (or `OBSIDIAN_RAW_DIR`) is treated as a source. After promoting a file to a proper wiki page, **archive then delete the original from `_raw/`**. Never leave promoted files in `_raw/` — they'll be double-processed on the next run.
...
**Archive before delete:** Before removing any promoted file from `_raw/`, zip all files processed in this run into a single archive and save it to `$OBSIDIAN_VAULT_PATH/_archives/`. This happens once per raw-mode run (not once per file) — collect the full list of promoted file paths first, then create a single zip:

```bash
# 1. Ensure the _archives directory exists
mkdir -p "$OBSIDIAN_VAULT_PATH/_archives"

# 2. Generate a timestamp for the archive name
STAMP=$(date +"%Y%m%d-%H%M%S")
ARCHIVE="$OBSIDIAN_VAULT_PATH/_archives/raw-processed-content-${STAMP}.zip"

# 3. Zip all promoted files in one pass (use -j to strip directory paths,
#    -- to guard against filenames starting with dashes)
zip -- "$ARCHIVE" "${PROMOTED_FILES[@]}"

# 4. Verify the archive was created and is non-empty before proceeding
if [ ! -s "$ARCHIVE" ]; then
  echo "ERROR: archive creation failed — aborting deletion of _raw/ files"
  exit 1
fi
```

Where `PROMOTED_FILES` is the array of absolute paths to every `_raw/` file promoted in this run. Always double-quote each path when building the array to handle filenames with spaces or special characters.

Only proceed to file deletion after the archive exists and passes the non-empty check. If the zip step fails, **do not delete** the source files — report the error to the user and leave `_raw/` intact.

Record the archive path in `log.md` alongside the ingest entry (see Step 7).

**Deletion safety:** Only delete the specific files that were promoted in this run. Before deleting each file, verify its resolved path is inside `$OBSIDIAN_VAULT_PATH/_raw/` — never delete files outside this directory. Never use wildcards or recursive deletion (`rm -rf`, `rm *`). Delete one file at a time by its exact path.
...
- Markdown (`.md`) — read directly. After reading, scan the full text for image references in any of these forms: `![[image.png]]`, `![alt](path.ext)`, `![](path.ext)`. For each reference found, resolve the image path relative to the source file's directory (or the vault root for wikilink-style references). If the image file exists on disk, load it using the Read tool and process it through the **Linked Image branch** below. If the image file cannot be found, retain the link as-is and note it as unresolved in the ingest report.
...
### Linked Image branch (images embedded in markdown)

When processing a markdown source that contains image references, handle each linked image as follows:

1. **Resolve the image path** — resolve relative to the source file's directory for markdown-style links (`![](path)`), or relative to the vault root for wikilink-style references (`![[image.png]]`).
2. **Copy to `_assets/`** — copy the image file into `$OBSIDIAN_VAULT_PATH/_assets/` using its original filename. Create the `_assets/` directory if it does not exist. If a file with the same name already exists in `_assets/`, append a datetime stamp to the new filename before copying — do not overwrite the existing file:
   ```bash
   mkdir -p "$OBSIDIAN_VAULT_PATH/_assets"

   DEST="$OBSIDIAN_VAULT_PATH/_assets/<filename>"
   if [ -f "$DEST" ]; then
     STAMP=$(date +"%Y%m%d_%H%M%S")
     # Insert stamp before extension: image.png → image_20260603_143012.png
     DEST="$OBSIDIAN_VAULT_PATH/_assets/<basename>_${STAMP}.<ext>"
   fi
   cp -- "<resolved-source-path>" "$DEST"
   ```
   The stamped filename (e.g. `image_20260603_143012.png`) becomes the canonical name used in both the copied file and the rewritten link. Always use `--` and double-quote paths to handle filenames with spaces or special characters.
3. **Rewrite the link** — replace the original image reference in the wiki page output with a wikilink pointing to the final destination filename (use the stamped name if one was generated in step 2):
   - `![[image.png]]` → `![[_assets/image.png]]` (or `![[_assets/image_20260603_143012.png]]` if stamped)
   - `![alt](path/to/image.png)` → `![[_assets/image.png]]` (or stamped equivalent)
   Always normalise to wikilink format regardless of how the original was written, unless `OBSIDIAN_LINK_FORMAT` is set to `markdown`, in which case rewrite as `![alt](_assets/<final-filename>)`.
4. **Load the image** — use the Read tool on the resolved source path to render it into your context.
5. **Transcribe and describe** — apply the same four steps from the Multimodal branch above: transcribe visible text verbatim, describe structure, extract concepts, note ambiguity.
6. **Append inline below the rewritten link** — immediately after the rewritten image reference in the wiki page output, insert a collapsible callout containing the transcription:

```markdown
![[_assets/image.png]]
> [!note]- Image transcript
> <transcribed text verbatim, if any>
>
> **Description:** <structural description — diagram nodes/edges, screenshot context, etc.> ^[inferred]
>
> **Concepts:** <comma-separated key concepts visible or inferred from the image> ^[inferred]
```

If the image contains no readable text (purely decorative or photographic), omit the "Image transcript" line and only include the Description and Concepts lines.

If the image file cannot be found or copied, retain the original link unchanged and append a brief note:

```markdown
![[image.png]]
> [!warning]- Image not processed
> Image could not be found at <resolved path> — link retained as-is.
```

If the image file was found but cannot be read by the model (unsupported format or no vision capability), copy it to `_assets/` and rewrite the link, but append:

```markdown
![[_assets/image.png]]
> [!warning]- Image not transcribed
> Image copied to _assets but could not be loaded for transcription.
```

Apply provenance markers (`^[inferred]`, `^[ambiguous]`) to interpreted content exactly as in the standalone Multimodal branch. Count image-derived claims when computing the `provenance:` frontmatter block for the page.
...
**YAML value quoting — CRITICAL:** Obsidian's YAML parser will fail silently and render the entire frontmatter block as plain text if a value contains unquoted special characters. Always wrap the value in double quotes when it contains any of the following:

| Character | Example |
|---|---|
| Em dash `—` (U+2014) | titles or summaries copied from source content |
| En dash `–` (U+2013) | date ranges, version ranges |
| Colon `:` | subtitles, URLs |
| `[` or `]` | unless the value is itself a YAML list |
| `{` or `}` | inline mappings |
| `#` at start of value | would be interpreted as a comment |
| `>` or `\|` | block scalar indicators |

This applies especially to `title:` and `summary:` fields, which frequently contain punctuation drawn from source content. When in doubt, quote the value — quoted strings are always safe.

```yaml
# WRONG — em dash in title breaks YAML parsing
title: My-PC — Storage Configuration

# CORRECT
title: "My-PC — Storage Configuration"
summary: "Four drives — NVMe C:, SATA HDD D:, two SATA SSDs E/F:."
```
...
- Check that any `title:` or `summary:` values containing special characters are quoted — fix them if not
...
For raw mode runs, include the archive path:
```
- [TIMESTAMP] INGEST source="_raw/" pages_updated=N pages_created=M mode=raw archive="path/to/_archives/raw-processed-content-YYYYMMDD-HHMMSS.zip"
```

If archiving failed, record it explicitly:
```
- [TIMESTAMP] INGEST source="_raw/" pages_updated=N pages_created=M mode=raw archive=FAILED reason="<short error>"
```
...
- [ ] *(Raw mode only)* A zip archive named `raw-processed-content-YYYYMMDD-HHMMSS.zip` exists in `_archives/` and is non-empty before any `_raw/` files were deleted
